...
SELECT customer_id, product_id, market_date, transaction_time, quantity, cost_to_customer_per_qty,
...
WITH daily_sales AS (
SELECT market_date,
SUM(quantity * COALESCE(cost_to_customer_per_qty, 0)) AS total_sales
FROM customer_purchases
...
SUM(5 * COALESCE(vi.original_price, 0)) AS total_revenue
FROM
(SELECT vendor_id, product_id, original_price FROM vendor_inventory) vi
